...
PSICRO_API t_ur_vau(double ur, double vau) {
    ...
    int iter = 0;
    double diff = 1.0;
    double dt = 0.00001; // Passo per la derivata numerica

    // 2. SOLUZIONE DI 1° TENTATIVO 
    if (vau >= 0.78) {
        t_curr = 5.0;
    }
    else {
    ...
    // 3. CICLO NEWTON-RAPHSON
    while (diff > epsilon && iter < max_iter) {
        // Calcolo f(t): usiamo la funzione volume specifico che hai già
        // f(t) = vau_calcolato(t, ur) - vau_target
        double vau_calc = vau_t_ur(t_curr, ur);
        double f_t = vau_calc - vau;

        // Calcolo f(t + dt) per la derivata numerica
        double vau_plus = vau_t_ur(t_curr + dt, ur);
        double f_t_dt = vau_plus - vau;

        // Calcolo della derivata numerica: [f(t+dt) - f(t)] / dt
        double df_dt = (f_t_dt - f_t) / dt;

        // Protezione contro la divisione per zero
        if (fabs(df_dt) < 1e-15) break;

        // Aggiornamento Newton: t_next = t_curr - f(t) / f'(t)
        double t_next = t_curr - f_t / df_dt;
...
PSICRO_API t_h_tbu(double h, double tbu){
    ...
    if (fabs(tbu) < 0.0001) {// le due eq vanno in crisi se tbu=0
        double xsat0 = xsat_t(0.0);
		double hsat_t0 = LAMBDA * xsat0; // Entalpia di saturazione a t=0°C
        // x da h
        double x = xsat0 - (hsat_t0 - h) / LAMBDA;
        // T da  h
        return (h - LAMBDA * x) / (CPAS + CPV * x);
    }
    else
    {
        double a, b;
        ...
        return (a * LAMBDA - h * b) / (CPV * (h - a) + CPAS * (LAMBDA - b)); }}
...
PSICRO_API t_tbu_tr(double tbu, double tr){ return t_x_tbu(x_t_ur(tr,100), tbu); }
...
PSICRO_API tbu_t_ur(double t, double ur) {
    if (((fabs(ur - 100.0))< 0.0001) || ur>=100) return t;
